...
class StandardQFNumeric(ps.BoundedInterestingnessMeasure):
    ...
    Computes interestingness as the difference between subgroup and dataset
    centroid (mean or median), weighted by subgroup *coverage* raised to 'a':

        phi(sg) = (n_sg / n_dataset)**a * |centroid_sg - centroid_dataset|

    Subgroups outside the coverage band [min_sg_coverage, max_sg_coverage]
    are discarded (quality 0, optimistic estimate -inf).

    Attributes:
        a (float): Exponent trading off subgroup coverage vs centroid difference.
    ...
    def standard_qf_numeric(
        a, instances_dataset, mean_dataset, instances_subgroup, mean_sg
    ):
        """Compute the standard quality function for numeric targets.

        Parameters:
            a (float): Exponent for weighting the subgroup coverage.
            instances_dataset (int): Number of instances in the dataset.
        ...
        """
        return (instances_subgroup / instances_dataset) ** a * (
            abs(mean_sg - mean_dataset)
        )
    ...
    def __init__(
        ...
        Raises:
            ...
            ValueError: If the coverage band is invalid.
        """
    ...
    def _coverage_ok(self, sg_size):
        ...
        cov = sg_size / self.dataset_statistics.size_sg
    ...
    def evaluate(self, subgroup, target, data, statistics=None):
        ...
        Coverage-weighted centroid difference; 0 if outside the coverage band.
    ...
    def calculate_statistics(
        ...
        Out-of-coverage subgroups get estimate=-inf so they are pruned/discarded.
        ...
        sg_centroid = 0
        if sg_size > 0 and self._coverage_ok(sg_size):
            sg_target_values = self.all_target_values[cover_arr]
            sg_centroid = self.agg(sg_target_values)
            # Safe, admissible (loose, non-pruning) optimistic estimate consistent
            # with the coverage-weighted reward. The original estimator theorems
            # assume an n**a reward and are not admissible for (n/N)**a, so they
            # are not used for the bound. Max coverage weight is max_sg_coverage;
            # max centroid gap is bounded by the dataset value range.
            value_range = float(
                np.max(self.all_target_values) - np.min(self.all_target_values)
            )
            estimate = (self.max_sg_coverage ** self.a) * value_range
        else:
            estimate = float("-inf")
    ...
    class Summation_Estimator:
        ...
        NOTE: derived for an n**a reward; retained for reference but not used by
        optimistic_estimate under the coverage-weighted formula.
        """
    ...
    class MeanOrdering_Estimator:
        ...
        Sorts the target values and computes the optimal subgroup by considering
        prefixes of the sorted list.
